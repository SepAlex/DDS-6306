...
# Unit 8
## 2DS only allows one file submitted - combined CaseStudy and Unit 8 in one file

Using the beer and breweries data files you provided, my team was able to gather some insights for Budweiser entering the craft beer market in the US. The analysis I will go through with you highlights (1) a look at the data, (2) Beers and breweries by state, (3) descriptive statistics for ABV, (4) median ABV & IBU values by state, (5) maximum ABV and IBU values by state and (6) the correlation between ABV and IBU. The purpose is to give you descriptive information about the current craft beer market in the US to help direct what type of craft beer to produce and where to market it. 
The data consists of 2410 craft beers from 558 US breweries. Fields that were available include: brewery, the state the brewery is in, the name of the beer, the style of the beer, ABV, IBU and ounces.
Diving deeper into missing variables we found that only ABV and IBU have missing values in the data provided. This chart shows the percentage of missing values by variable and state. The lighter the color, the more missing values. Two things to note on this chart: the first is that Delaware appears to have the same % of missing values for ABV and IBU. Digging further into that it turns out that if ABV is missing on any record, the IBU is also missing. The other thing to note is that South Dakota does not have any records where the IBU value is populated.
Next the team looked at Breweries by state and beer trends. This chart shows brewery count by state. 5 states with the most number of breweries is: Colorado, California, Michigan, Oregon and Texas. All states have at least one craft brewery in the dataset. 33 states have 10 or fewer breweries. The purpose of this chart is to talk about market saturation. The more saturated a market is the more difficult it may be to enter the market. More research on overall beer consumption compared to craft beer consumption by market is recommended to help identify an ideal place to enter the market.
As far as what types of beer to produce, this chart shows the top 10 craft beer styles that are being produced. We are using a measure of popularity based on the number of beers produced, not consumption. This shows popularity of beers in each market.
It is important to know some information about the properties of the beer. What you see here is statistics for ABV. The median is…
Now we will look at the median ABV and IBU by state. You will see that there is a wider range of median IBU values by state that ABV. 
Max ABV and IBU
Correlation between IBU and ABV
To wrap up, this analysis provided descriptive information about the craft beer market in the US. We highlighted the 10 most popular beers that you should consider for production. We highlighted the middle 50% range for ABV of .056 and .060 that you should aim for during production. We do feel that more research needs to be done before determining which market to target first. Thank you for your time.

https://youtu.be/R0dN3HepWeI
